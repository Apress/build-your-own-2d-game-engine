...
Camera.prototype.setWCCenterNow = function (xPos, yPos) {
    var p = vec2.fromValues(xPos, yPos);
    this.mCameraState.setCenterNow(p);
};
...
Camera.prototype.objectOutOfScope = function (aXform) {
    var bbox = new BoundingBox(aXform.getPosition(), aXform.getWidth(), aXform.getHeight());

    var cameraBound = new BoundingBox(this.getWCCenter(), this.getWCWidth(), this.getWCHeight());
    
    if(bbox.maxX() < cameraBound.minX()) { return true; }
    else { return false; }
};
