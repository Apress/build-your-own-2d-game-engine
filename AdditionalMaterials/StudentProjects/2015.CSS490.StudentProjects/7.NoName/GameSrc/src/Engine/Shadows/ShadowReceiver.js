...
ShadowReceiver.prototype.removeShadowCaster = function (lgtRenderable) {
    var i;
    for(i = this.mShadowCaster.length - 1; i >= 0; i--) {
        if(this.mShadowCaster[i].getShadowCaster() === lgtRenderable) {
            this.mShadowCaster.splice(i, 1);
        }
    }
};
...
// to allow lights to dynamically be added to cast light on the shadow receiver

ShadowReceiver.prototype.addLight = function (l) {
    this.mReceiver.addLight(l);
};

ShadowReceiver.prototype.removeLight = function (l) { 
    this.mReceiver.removeLight(l); 
};
// </editor-fold>