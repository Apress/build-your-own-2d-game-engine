...
Transform.prototype.setXform = function(xf) {
  var vec = vec3.fromValues(xf[0], xf[1], xf[2]);
  var sx = Math.round(vec3.length(vec) * 1000) / 1000;

  vec = vec3.fromValues(xf[4], xf[5], xf[6]);
  var sy = Math.round(vec3.length(vec) * 1000) / 1000;

  var rot1 = vec3.fromValues(xf[0] / sx, xf[1] / sx, xf[2] / sx);
  var az = Math.atan2(rot1[1], rot1[0]);
  
  this.setPosition(xf[12], xf[13]);
  this.setSize(sx, sy);
  this.setRotationInRad(az);
};
//</editor-fold>