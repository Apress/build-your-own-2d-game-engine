...
function BoundingBox(centerPos, w, h) {
    ...
    this.mBound = new TextureRenderable(gEngine.DefaultResources.getBoundTexture());
    this.mBound.getXform().setPosition(centerPos[0], centerPos[1]);
    this.mBound.getXform().setSize(w, h);
}
...
BoundingBox.prototype.getBound = function() { return this.mBound; };

// Added this so that BoundingBox can be drawn as a group
BoundingBox.prototype.draw = function (camera){
    this.mBound.draw(camera);
};

// Added this so that BoundingBox can be drawn as a group
BoundingBox.prototype.drawAsChild = function (camera, groupTransform){
    this.mBound.drawAsChild(camera, groupTransform);
};
