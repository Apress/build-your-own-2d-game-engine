 ...
 * Description:		Defines the functions that supports camera manipulations
 */
...
Camera.prototype.update = function (cameraBound) {
    ...
    // this does interpolation first
    this.mCameraState.updateCameraState();
    
    // Added new variables for calculations to sets up camera bounds
    var curCenter = this.mCameraState.getCenter();
    var newCenter = vec2.clone(this.getWCCenter());
    
    if (curCenter[0] + this.getWCWidth()/ 2 > cameraBound[1]){
        newCenter[0] = cameraBound[1] - this.getWCWidth() / 2;
    }
    if (curCenter[0] - this.getWCWidth()/ 2 < cameraBound[0]){
        newCenter[0] = cameraBound[0] + this.getWCWidth() / 2;
    }
    if (curCenter[1] + this.getWCHeight()/ 2 > cameraBound[2]){
        newCenter[1] = cameraBound[2] - this.getWCHeight() / 2; 
    }
    if (curCenter[1] - this.getWCHeight()/ 2 < cameraBound[3]){
        newCenter[1] = cameraBound[3] + this.getWCHeight() / 2; 
    }
    
    // Added the so that the it will set the camera state to the current
    // so that it does not go out of bounds after interpolation
    this.mCameraState.setCurrentCenter(newCenter);
};
