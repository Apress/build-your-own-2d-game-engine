...
GameObject.prototype.removeAllLights = function () {
    this.mRenderComponent.removeAllLights();
};
GameObject.prototype.removeLightAt = function (index) {
    this.mRenderComponent.removeLightAt(index);
};
...
// Orientate the entire object to point towards point p
// will rotate Xform() accordingly
GameObject.prototype.changeTravelDirection = function (p, rate) {
    // Step A: determine if reach the destination position p
    var dir = [];
    vec2.sub(dir, p, this.getXform().getPosition());
    var len = vec2.length(dir);
    ...
    //this.getXform().incRotationByRad(rad); //DON'T turn the visual object.
};
