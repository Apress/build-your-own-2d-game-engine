...
BoundingBox.prototype.enterCollideStatus = function (otherBound) {
    ...
    if (this.intersectsBound(otherBound)) {
        if (this.minY() > otherBound.minY() && this.maxY() > otherBound.maxY())
            status += 1;
        if (this.maxY() < otherBound.maxY() && this.minY() < otherBound.minY())
            status += 2;
        if (this.minX() > otherBound.minX() && this.maxX() > otherBound.maxX())
            status += 4;
        if (this.maxX() < otherBound.maxX() && this.minX() < otherBound.minX())
            status += 8;
    }
    if (status === 5)
    {
        if (otherBound.maxY() - this.minY() < otherBound.maxX()-this.minX())
            status = 1;
        else status = 4;
    }
    if (status === 9)
    {
        if (otherBound.maxY() - this.minY() < this.maxX() - otherBound.minX())
            status = 1;
        else status = 8;
    }
    if (status === 6)
    {
        if (this.maxY() - otherBound.minY() < otherBound.maxX() - this.minX())
            status = 2;
        else status = 4;
    }
    if (status === 10)
    {
        if (this.maxY() - otherBound.minY() < this.maxX() - otherBound.minX())
            status = 2;
        else status = 8;
    }
...
// BoundingBox.prototype.enterCollideStatus = function (otherBound,speedX,speedY,flag){
//     var status = 0;
//     // status:
//     // 1: onTop
//     // 2: onBottom
//     // 4: onRight
//     // 8: onLeft
//     if (this.intersectsBound(otherBound) && !flag)
//     {
//         if (this.maxY()>otherBound.maxY())
//             return 16;
//         if (this.minY()<otherBound.minY())
//             return 17;
//     }
//     if (this.minY()+speedY < otherBound.maxY() && this.minY()>otherBound.maxY() && this.minX()<otherBound.maxX() && this.maxX()>otherBound.minX())
//         status += 1;
//     if (this.maxY()+speedY > otherBound.minY() && this.maxY()<otherBound.minY() && this.minX()<otherBound.maxX() && this.maxX()>otherBound.minX())
//         status += 2;
//     if (this.minX()+speedX < otherBound.maxX() && this.minX()>otherBound.maxX() && this.minY()<otherBound.maxY() && this.maxY()>otherBound.minY())
//         status += 4;
//     if (this.maxX()+speedX > otherBound.minX() && this.maxX()<otherBound.minX() && this.minY()<otherBound.maxY() && this.maxY()>otherBound.minY())
//         status += 8;
//     return status;
// }
