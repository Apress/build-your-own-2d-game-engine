...
GameObject.prototype.moveTowards_H = function (target, moveSpeed) {
    var thisXPos = this.getXform().getXPos();
    var targetXPos = target.getXform().getXPos();
    if(Math.abs(thisXPos - targetXPos) >= 0.5) {
        this.getXform().incXPosBy(Math.sign(targetXPos - thisXPos) * moveSpeed);
    }
};
GameObject.prototype.moveTowards_V = function (target, moveSpeed) {
    var thisYPos = this.getXform().getYPos();
    var targetYPos = target.getXform().getYPos();
    if (Math.abs(thisYPos - targetYPos) >= 0.5) {
        this.getXform().inYPosBy(Math.sign(targetYPos - thisYPos) * moveSpeed);
    }
};

GameObject.prototype.moveAlong_X = function(moveSpeed) {
    this.getXform().incXPosBy(moveSpeed);
};
GameObject.prototype.moveAlong_Y = function(moveSpeed) {
    this.getXform().incYPosBy(moveSpeed);
};