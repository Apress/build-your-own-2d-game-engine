...
'use strict'
/*global gEngine*/ 
function StartGame3(player1,player2,player1Texture,player2Texture,player1WinTimes = 0, player2WinTimes = 0){
    ...
    this.mBackgroundTexture = 'assets/map3.png'
}

gEngine.Core.inheritPrototype(StartGame3, Scene)

StartGame3.prototype.generateRandomObstacle = function(){
    let random = (min, max) => {
...
StartGame3.prototype.loadScene = function () {
...
StartGame3.prototype.unloadScene = function () {
    ...
    if(this.mPlayer1WinTimes === 3){
        ...
        next = new EndScene3(this.mResult,this.kPlayer2Texture,true,this.mPlayer1Character,this.mPlayer2Character)
    }else if(this.mPlayer2WinTimes === 3){
        this.mResult = this.kPlayer2Texture
        next = new EndScene3(this.mResult,this.kPlayer1Texture,false,this.mPlayer1Character,this.mPlayer2Character)
    }else{
        next = new StartGame3(this.mPlayer1Character,this.mPlayer2Character,this.kPlayer1Texture,this.kPlayer2Texture,
...
StartGame3.prototype.initialize = function(){
    let canvas = document.getElementById('GLCanvas')
    canvas.width = '1200'
    canvas.height = '850'
    ...
    this.mPlayer1.getXform().setPosition(-18,0)
    this.mPlayer2 = new Player2(this.kPlayer2Texture,this.mPlayer2Character)
    this.mPlayer2.getXform().setPosition(18,0)
    ...
    this.mGameArea.setColor([0,1,0,0])
...
StartGame3.prototype.draw = function(){
...
StartGame3.prototype.update = function(){
  ...
  //  gEngine.Physics.processCollision(this.mObstacles.concat(this.mBoundSet),this.mCollisionInfos)
    ...
    let distance1 = Math.sqrt(pos1[0]*pos1[0] + pos1[1] * pos1[1])
    let pos2 = this.mPlayer2.getXform().getPosition()
    let distance2 = Math.sqrt(pos2[0]*pos2[0] + pos2[1] * pos2[1])
    let r = 25
    if(distance1 > r){
