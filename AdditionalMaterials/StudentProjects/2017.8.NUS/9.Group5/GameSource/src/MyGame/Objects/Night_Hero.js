...
function NightHero( spriteTexture) {
    this.kDelta = 0.3;
...
gEngine.Core.inheritPrototype(NightHero, GameObject);

NightHero.prototype.update = function (mBullet, Bflag, FireCD,mTheLight) {
    // control by WASD
   ...
   gEngine.Core.startScene(nextLevel);
    ...
    if(this.mEXP>= 500){
   var nextLevel = new Win();  // load the next level
   gEngine.Core.startScene(nextLevel);
}
};

NightHero.prototype.getHP = function(){
    return this.mHP;
};

NightHero.prototype.decHP = function(deltaHp){
    this.mHP -= deltaHp;
};

NightHero.prototype.incHP = function(deltaHp){
    this.mHP += deltaHp;
};
NightHero.prototype.getEXP = function(){
    return this.mEXP;
};

NightHero.prototype.incExp = function(deltaExp){
    this.mEXP += deltaExp;
};

NightHero.prototype.decEXP = function (deltaEXP) {
    this.mEXP -= deltaEXP;
};

NightHero.prototype.incSpeed = function(deltaExp){
    this.kDelta += deltaExp;
};
NightHero.prototype.getSpeed = function(){
    return this.kDelta;
};