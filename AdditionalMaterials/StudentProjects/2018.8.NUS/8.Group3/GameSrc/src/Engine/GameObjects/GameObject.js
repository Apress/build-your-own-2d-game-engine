...
GameObject.prototype.incVelocity = function (delta) {
    var curSpeed = Math.max(this.getVelocity()+delta,0);
    vec2.scale(this.mRigidBody.mVelocity,this.mCurrentFrontDir,curSpeed);
};

GameObject.prototype.getVelocity = function () {
    return vec2.length(this.mRigidBody.mVelocity);
};

GameObject.prototype.setVelocity = function (Speed) {
    vec2.scale(this.mRigidBody.mVelocity,this.mCurrentFrontDir,Speed);
};