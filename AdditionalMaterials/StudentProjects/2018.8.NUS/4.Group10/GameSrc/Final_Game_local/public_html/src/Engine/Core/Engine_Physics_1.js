...
gEngine.Physics = (function () {
    ...
    var mSystemtAcceleration = [0, -600];        // system-wide default acceleration
        ...
        while (continueRelaxation()) {
            for (i=0; i<set.size(); i++) {
                ...
                if ((s1 !== s2) && (s1.collided(s2, mCollisionInfo))) {
                    //Judge if it is skip this wall
                    if(obj.mJumpSkip===null)
                        resolveCollision(s1, s2, mCollisionInfo);
                    else if(obj.mJumpSkip===true && mCollisionInfo.getNormal()[1]>0&& mCollisionInfo.getNormal()[0]===0){
                        obj.mJumpSkip=s2;
                    }
                }
                else if (obj.mJumpSkip===s2)
                    obj.mJumpSkip=null;
            }
        }
    };
