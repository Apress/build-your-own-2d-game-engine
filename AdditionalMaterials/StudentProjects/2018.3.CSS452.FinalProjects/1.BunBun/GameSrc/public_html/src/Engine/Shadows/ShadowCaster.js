...
ShadowCaster.prototype._computeShadowGeometry = function(aLight) {
    ...
    var cxf = this.mShadowCaster.getTransform();
    var rxf = this.mShadowReceiver.getTransform();
...
ShadowCaster.prototype.draw = function(aCamera) {
    ...
    this.mShadowCaster.getTransform().cloneTo(this.mSaveXform);
    ...
    for (l = 0; l < casterRenderable.numLights(); l++) {
        ...
        if (lgt.isLightOn() && lgt.isLightCastShadow()) {
            this.mSaveXform.cloneTo(this.mShadowCaster.getTransform());
