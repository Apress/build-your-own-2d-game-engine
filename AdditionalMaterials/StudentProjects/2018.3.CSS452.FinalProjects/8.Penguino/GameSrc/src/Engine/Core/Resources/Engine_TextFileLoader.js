...
gEngine.TextFileLoader = (function () {
    ...
    var saveTextFile = function (fileName, fileType, callbackFunction, msg) {
        if (gEngine.ResourceMap.isAssetLoaded(fileName) || true) {
            ...
            req.onreadystatechange = function () {
                console.log(req);
                if ((req.readyState === 4) && (req.status !== 200)) {
                    alert(fileName + ": saving failed! [Hint: you cannot double click index.html to run this project. " +
                        "The index.html file must be loaded by a web-server.]");
                }
            };
            req.open('PUT', "localhost:8383/Penguino/" + fileName, true);
