...
gEngine.Physics = (function () {
    ...
    var setSystemtAcceleration = function(ax, ay)
    {
      mSystemtAcceleration[0] = ax;
      mSystemtAcceleration[1] = ay;
    };
    ...
    var processCollisionPair = function(set, os, infoSet) {
        ...
        for (r= 0; r<mRelaxationCount; r++) {
            for (i = 0; i<set.size(); i++) {
                ...
                if(objI==null) break;
                for (j = 0; j<os.size(); j++) {
                    var objJ = os.getObjectAt(j).getRigidBody();
                    if(objJ==null) break;
                    if ((objI !== objJ) && ((objI.getInvMass() !== 0) || (objJ.getInvMass() !== 0)) ) {
                        if (objI.boundTest(objJ)) {
                            if (objI.collisionTest(objJ, info)) {
                                // make sure info is always from i towards j
                                vec2.subtract(iToj, objJ.getCenter(), objI.getCenter());
    ...
    var mPublic = {
        ...
        setSystemAcceleration: setSystemtAcceleration,
        processCollision: processCollision,
        processCollisionPair: processCollisionPair,
