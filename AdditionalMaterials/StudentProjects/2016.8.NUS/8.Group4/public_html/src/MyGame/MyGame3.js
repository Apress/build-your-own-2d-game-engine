...
function MyGame3() {
    ...
    this.kFloor5 = "assets/Floor7.png";
    //音乐
    this.kGameBGSong = "assets/BGClip3.mp3";
...
MyGame3.prototype.initialize = function () {    
    ...
    this.mLevel = new FontRenderable("Level 3");
    ...
    this.mGunName = new FontRenderable("Enemy :  "+ this.countEnemy+"/30");
    this.mGunName.setColor([0, 0, 0, 1]);
    this.mGunName.getXform().setPosition(25, 108);
    this.mGunName.getXform().setSize(12,5);
    
    this.mGunName2 = new FontRenderable("SuperEnemy :  "+this.countSuperEnemy+"/20");
...
MyGame3.prototype.drawCamera = function (camera) {
...
MyGame3.prototype.update = function () {
    ...
    this.mGunName.setText("Enemy :  "+ this.countEnemy+"/30");
    this.mGunName2.setText("SuperEnemy :  "+ this.countSuperEnemy+"/20");
    this.mHero.update();
    this.mCamera.update();  
    ...
    for(i = 0; i<this.mBullet.length;i++){
        ...
        for(j = 0; j < this.mSuperEnemy.length; j++){
            ...
            this.mGunName2.setText("Enemy : "+ this.countSuperEnemy+"/20");
        }
        }
    }
    for(i = 0;i<this.mEnemyBullet.length;i++){
    ...
    for(i = 0; i < this.mEnemy.length; i++){
        ...
        for(j = 0; j < this.mEnemy.length; j++){
            if(this.mEnemy[i].getXform().getXPos() > this.mHero.getXform().getXPos() - 9 
                ...
                && this.mEnemy[i].getXform().getYPos() > this.mHero.getXform().getYPos() - 18
                && this.mEnemy[i].getXform().getYPos() < this.mHero.getXform().getYPos() + 18){
                var t = this.mEnemy[0];
    ...
    for(i = 0; i < this.mSuperEnemy.length; i++){
        ...
        for(j = 0; j < this.mSuperEnemy.length; j++){
            if(this.mSuperEnemy[i].getXform().getXPos() > this.mHero.getXform().getXPos() - 9 
                ...
                && this.mSuperEnemy[i].getXform().getYPos() > this.mHero.getXform().getYPos() - 22
                && this.mSuperEnemy[i].getXform().getYPos() < this.mHero.getXform().getYPos() + 22){
  ...
  /* if(xform.getXPos() - this.pastZ > 160 || this.pastZ - xform.getXPos() > 160){
    ...
    if(this.countEnemy >= 30 && this.countSuperEnemy >= 20){
        var nextLevel = new BossGame();  // load the next level
        gEngine.Core.startScene(nextLevel);
    }
